...
//
 ...
 jQuery(document).on('change', 'input[type=radio][name=woofood_order_type]', function (){
  ...
  woofood_multi_hide_address_on_pickup();
      ...
      // jQuery('#wf_availability_popup').modal('show');
...
"><?php _e('Confirmar Ubicación', 'woofood-plugin'); ?></button>
...
else
{
  ...
  $response = '<div class="availability-result">'.$svg_sad.'<div class="availability-result-message">'.sprintf(esc_html__(" %s no está disponible", "woofood-plugin").$additional_error.'</div>', woofood_get_order_type_by_key($order_type)).'</div>';
