...
class LicenseBoxAPI {
  ...
  { 
    $this->product_id = '1027BBC5';
    $this->api_url = 'https://licensing.wpslash.com/';
    $this->current_version = '1.3.1';
    $this->current_path = realpath(__DIR__);
    $this->root_path = realpath($this->current_path.'/..');
    $this->verify_type = 'envato';
    $this->api_key = '9C13094C311C066395D2';
  ...
  public function verify_license($time_based_check = false, $license = false, $client = false){
    ...
    $today = date('d-m-Y');
    if(empty($_SESSION["73f5269374"])){
      $_SESSION["73f5269374"] = '0000-00-00';
    }
    if($type==1){
    if($today>=$_SESSION["73f5269374"]){
